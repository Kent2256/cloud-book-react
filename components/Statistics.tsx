    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-24">
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl p-4 shadow-sm border border-slate-100 dark:border-slate-800 space-y-4">
        ...
        <div className="flex items-center justify-between">
            <button onClick={() => changeDate(-1)} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full transition-colors">
            ...
            <div className="flex gap-2">
                <button 
                    ...
                    className={`p-2 rounded-full transition-colors relative ${showFilter || hasActiveFilters ? 'bg-indigo-100 text-indigo-600 dark:bg-indigo-900/50 dark:text-indigo-400' : 'hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-400'}`}
                >
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-1 hide-scrollbar">
                        ...
                        {users.map(u => (
                            <button
                                ...
                                className={`px-3 py-1.5 rounded-full text-xs font-medium border whitespace-nowrap flex items-center gap-1 transition-all ${selectedMemberId === u.uid ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300'}`}
                            >
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-1 hide-scrollbar">
                        ...
                        {availableFilterCategories.map(cat => (
                            <button
                                ...
                                className={`px-3 py-1.5 rounded-full text-xs font-medium border whitespace-nowrap transition-all ${filterCategory === cat ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300'}`}
                            >
                                {cat}
                            </button>
                        ))}
