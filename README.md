...
### v3.4.1
- 回歸用戶引導：登入後可選擇已加入帳本，避免本機資料遺失造成卡關。
- 設定頁新增「目前帳本」切換器，快速切換帳本。
- 文字修改

### v3.4.0
- 新手引導（後端）：新增使用者建立觸發器，預設 `ledgers` 為空陣列並停用自動建立帳本。
- 新手引導（前端）：登入後若無帳本顯示初始設定頁，可建立或加入帳本。
- 帳本生命週期（後端）：新增加入/退出帳本與排程清理機制。
- 資安修正：Firebase Hosting 強制 HTTPS 重導向。
